...
const BoostTimeFarm = () => {
  ...
  useEffect(() => {
    setMiningTimeList(
      boostTimeLevels.map((item) => ({
        ...item,
        capacity: initialMiningTime * item.capacity,
      }))
    );
  }, [initialMiningTime, boostTimeLevels]);
      ...
      sendTransaction(transaction, {
        onSuccess: async (hash) => {
          ...
          await updateDoc(userRef, {
            boostedTime: newMiningTime,
          });
          setBoostedTime(newMiningTime);
          ...
          setCongratsMessage(
            <div className="w-full flex justify-center flex-col items-center space-y-3">
              ...
              <span className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2 flex flex-col justify-center w-full text-center items-center space-x-1">
                ...
                <span>
                  Mining Time Upgraded to {newMiningTime} Proft Per Hour
                </span>
              </span>

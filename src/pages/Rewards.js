...
const ReferralRewards = () => {
  ...
  const handleClaim = async (reward) => {
    if (
      ...
      setModalMessage(
        "You have already claimed this referral reward or do not meet the requirements."
      );
      setModalOpen(true);
    }
  };
      ...
      <div className="w-full pt-2 justify-center flex-col space-y-6 px-5">
        ...
        <div className="w-full flex items-center justify-between">
          <div
            ...
            }  rounded-[6px] text-[#c6c6c6] py-[10px] text-nowrap barTitle px-3 w-[45%] flex space-x-2 justify-center text-center text-[15px] font-semibold items-center`}
          >
          ...
          <div
            ...
            className={`${
              activeIndex === 2 ? "bg-cards3 text-[#ebebeb]" : ""
            } barTitle rounded-[6px] text-[#c6c6c6] py-[10px] px-3 w-[45%] space-x-2 font-semibold text-[15px] flex justify-center text-center items-center`}
          >
        ...
        <div
          id="refer"
          className="w-full h-[70vh] scroller rounded-[10px] overflow-y-auto pt-2 pb-[180px]"
        >
          <div
            className={`${
          ...
          >
            {friendsRewards
              .filter(
                (reward) => !claimedReferralRewards.includes(reward.title)
              )
              .map((reward) => {
                const progress =
                  (referrals.length / reward.referralsRequired) * 100;
                const isClaimable =
                  referrals.length >= reward.referralsRequired &&
                  !claimedReferralRewards.includes(reward.title);
                return (
